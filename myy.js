...
async function delod () {
 ...
 if(Object.entries(selod5)[0]){
  ...
  let text = "Want to delete? order No."+r.slice(3);
  ...
  //  st.collection(selg).doc(r).get().then(doc => {
    await mthdb(r.slice(2,6));
    await oddb.od.get(Number(r.slice(3))).then((doc)=>{
...
//  st.collection(selg).doc(r).update(shod11.od).then(response => {
  await oddb.od.put(shod11.od,Number(r)).then(response => {
    selod5={};
  ...
  window.indexedDB.databases().then((e)=>{
    e.forEach((v)=>{
         if (String(v.name).match(/\w\d{3}/g)) {
          let x=(2+v.name.slice(1));
          let mth=new Date('20'+x[0]+x[1],Number(x[2]+x[3])-1).toLocaleDateString('en-GB', {
            month : 'short',
            year : 'numeric'
            });
          // console.log(v.name,mth);
          document.getElementById('oderli').innerHTML+=`<ul id="${v.name}" class="w3-border w3-ul"><li onclick="getmth('${v.name}','${mth}')" class="w3-block w3-blue-gray w3-button">${mth}</li></ul>`;
         }
    });
  });
}
...
async function getmth(b,m){
 ...
 for (let u in selod5) {document.getElementById(u).checked=false;}selod={};
